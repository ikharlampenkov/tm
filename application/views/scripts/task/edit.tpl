...
<form action="{$this->url(['controller' => $controller,'action' => 'edit', 'id' => $task->id])}" method="post" enctype="multipart/form-data">
    ...
    {if $documentList !== false}
        {foreach from=$documentList item=document}
            ...
            <td class="ttovar"><a href="/files/{$document->file->getName()}" target="_blank">{$document->title}</a> / <a href="{$this->url(['controller' => 'document','action' => 'edit', 'id' => $document->id])}">редактировать</a> </td>
        </tr>
        {/foreach}
    {/if}
        <tr>
