                    ...
                    <div class="task_action">
                        {if !$task->hasParent()}
                            {if_allowed resource="{$controller}/fromArchive"}
                                <a href="javascript:void(0)" onclick="task.fromArchive('{$this->url(['controller' => $controller,'action' => 'fromArchive', 'idTask' => $task->id])}', 0, '{$this->url(['controller' => $controller,'action' => 'showArchiveBlock', 'parent' => 0])}')"><img src="/i/zip.gif" alt="из архива" title="из архива" border="0"/></a>
                            {/if_allowed}
                        {/if}
