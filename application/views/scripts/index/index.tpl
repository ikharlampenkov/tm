                        ...
                        <li class="discussion_list">
                            <div class="discussion_block">
                                ...
                                <div class="discussion_info">
                                    ...
                                    {if $discussion->isRequest}{if $discussion->isComplete}<img src="/i/is_complite.png" title="Выполнена" alt="Выполнена" border="0"/>{elseif $discussion->user->id==$authUserId}<button onclick="comment_complete_rq('{$this->url(['controller' => $controller,'action' => 'index', 'idTask' => $task->id, 'is_complete' => $discussion->id])}');">Завершить</button>{/if}{/if}
                                    {if $discussion->user->id==$authUserId}<button onclick="comment_complete_rq('{$this->url(['controller' => $controller,'action' => 'index', 'idTask' => $task->id, 'delete' => $discussion->id])}');">удалить</button> {/if}
                                </div>
                            </div>
                        </li>
                    {/foreach}
