                    ...
                    <td class="tedit">
                        {if $document->isFolder}
                            ...
                            {if_object_allowed type="{$controller|capitalize}" object="{$document}" priv="write"}
                                <img src="/i/edit.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'editFolder', 'id' => $document->id])}">редактировать</a>
                                <br/>
                            {/if_object_allowed}
                            {/if_allowed}
                        ...
                        {else}
                            ...
                            <img src="/i/document.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'view', 'id' => $document->id])}">просмотреть</a>
                            <br/>
                        {/if_allowed}
                            {if_allowed resource="{$controller}/edit"}
                        {if_object_allowed type="{$controller|capitalize}" object="{$document}" priv="write"}
                            <img src="/i/edit.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'edit', 'id' => $document->id])}">редактировать</a>
                            <br/>
                        {/if_object_allowed}
                        {/if_allowed}
                        {if $document->user->id == $authUserId || $authUserRole == 'admin'}
